...
import com.endsh.internal.util.ArrayUtils;
import com.endsh.text.style.AlignmentSpan;
import com.endsh.text.style.LeadingMarginSpan;
...
public abstract class Layout {
    ...
    public int getOffsetForHorizontal(int line, float horiz) {
        ...
        if (mText instanceof Spanned) {
        	ViewSpan[] vsp = ((Spanned) mText).getSpans(min, max, ViewSpan.class);
        	if (vsp.length == 1 && max < ((Spanned) mText).getSpanEnd(vsp[0])) {
        		max = ((Spanned) mText).getSpanEnd(vsp[0]);
        	}
        }
