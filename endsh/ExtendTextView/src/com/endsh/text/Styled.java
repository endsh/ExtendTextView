...
public class Styled {
	 ...
	 * interest should start or end in the middle of this run (if not drawing,
	 * character spans that don't affect metrics can be ignored). Neither should
	 * the run direction change in the middle of the run.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * The returned width is signed and will be < 0 if the paragraph direction
	 * is right-to-left.
	 */
	private static float drawUniformRun(Canvas canvas, Spanned text, int start,
		...
		if (replacement == null) {
			...
			if (canvas != null) {
				if (replacement instanceof ViewSpan) {
					((ViewSpan) replacement).layout((int) x, top);
				}
	 ...
	 * @param workPaint
	 *            a paint to modify; on return will reflect the original paint
	 *            plus the effect of all spans on the run
	 * @param text
	 *            the text
	 * @param start
	 ...
	 */
	public static int getTextWidths(TextPaint paint, TextPaint workPaint,
			Spanned text, int start, int end, float[] widths,
			Paint.FontMetricsInt fmi) {
		MetricAffectingSpan[] spans = text.getSpans(start, end,
	 ...
	 * 
	 * <p>
	 * This optimizes for unstyled text and so workPaint might not be modified
	 * by this call.
	 * 
	 * <p>
	 * The returned advance width will be < 0 if the paragraph direction is
	 * right-to-left.
	 */
	 ...
	 */
	/* package */static float drawText(Canvas canvas, CharSequence text,
			int start, int end, int dir, boolean runIsRtl, float x, int top,
			int y, int bottom, TextPaint paint, TextPaint workPaint,
			boolean needWidth) {
	...
	/**
	 * Draws a run of text on a single line, with its origin at (x,y), in the
	 ...
	 * @param direction
	 *            The direction of the text. This must be
	 ...
	 * @param workPaint
	 *            The {@link TextPaint} object used for temporal workspace.
	 * @param needWidth
	 *            If true, this method returns the width of drawn text
	 * @return Width of the drawn text if needWidth is true
	 */
	public static float drawText(Canvas canvas, CharSequence text, int start,
			int end, int direction, float x, int top, int y, int bottom,
		...
		return drawText(canvas, text, start, end, direction, false, x, top, y,
				bottom, paint, workPaint, needWidth);
	}
	 ...
	 * @param workPaint
	 *            the {@link TextPaint} object available for modification; will
